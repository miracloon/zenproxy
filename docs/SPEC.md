...
- GitHub Actions 按分支语义分为两类：
  - **验证型 CI**：面向 `dev`，只验证，不发布；
  - **发布型 CI**：面向 `main` / 版本 tag，负责正式镜像构建与推送。
- 不应把 `dev` 分支的 push 与正式镜像发布绑定到同一语义层。
- 若需要修改 GitHub Actions，应优先维护这种边界，而不是让 `dev` / `main` 共享同一条“自动发布”路径。

### AI 命令文档

- 仓库根目录允许放置少量 **供 AI 直接读取的命令文档**，作为高频维护动作的快捷入口。
- 这类文档的职责是：
  - 描述触发意图；
  - 定义进入动作前必须执行的检查；
  - 约束边界与停止条件；
  - 指向应执行的 git / 验证步骤。
- 这类文档不是长期控制正文，不替代 `AGENTS.md`、`INTENT.md`、`WORKFLOW.md`、`SPEC.md`。
- 文件应保持轻量、单任务、可直接阅读；适合放在仓库根目录，便于 AI 在进入高频操作前快速发现。

### 配置文件

- 服务端配置：TOML 格式（`config.toml`）
- 客户端配置：JSON 格式（`config.json`，sing-box 原生格式）
- Docker 环境变量：直接在 `docker-compose.yml` 中声明

### 网络模式

Docker 容器统一使用 `network_mode: host`。端口范围由各自配置文件管理，不通过 Docker 端口映射控制。
...
| 客户端代理池端口 | 60001-65535（fork 默认） | 环境变量 `PROXY_PORT_START` / `PROXY_PORT_END` |

### 端口同步模式

客户端支持通过环境变量 `SYNC_REMOTE_PORT=true` 或 Fetch API 参数 `sync_remote_port` 开启端口同步模式。
开启后，客户端 Fetch 时将使用远端代理的端口号创建本地绑定，`PROXY_PORT_START/END` 失效（可配置但不生效）。
端口冲突时报错跳过，不回退到自动分配。
