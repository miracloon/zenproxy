...
func NewServer(ctx context.Context, logFactory log.ObservableFactory, options option.ClashAPIOptions) (adapter.ClashServer, error) {
	...
	chiRouter.Group(func(r chi.Router) {
		...
		r.Mount("/bindings", bindingRouter(s.bindingManager))
		r.Mount("/store", proxyManageRouter(s.bindingManager))
		r.Mount("/fetch", fetchRouter(s.bindingManager))
		r.Mount("/subscriptions", subscriptionRouter(s.bindingManager))
